	...
	namespace PPGUI_PP_GUI {
		...
		void WindowCommand::Draw() {
			...
			RENDER::DrawGradientRectangle(
				...
				CColor(20, 20, 20, 255), CColor(21, 21, 21, 255));
			...
			std::string name1 = "			     exodos-";

			Vector2D textsize = RENDER::GetTextSize(info.font, name1);

			RENDER::DrawF(position.x + 34, position.y + (WINDOW_TITLE_BAR_SIZE * 0.5f) + 2,
				info.font, true, true, colors[WINDOW_TEXT], name1);

			std::string name2 = "			                   CSGO CHEAT";

			Vector2D textsize2 = RENDER::GetTextSize(info.font, name2);

			RENDER::DrawF(position.x + 0, position.y + (WINDOW_TITLE_BAR_SIZE * 0.5f) + 2,
				info.font, true, true, colors[GAME_SNEEZE], name2);

			//	RENDER::DrawF(position.x + textsize.x + 20, position.y + (WINDOW_TITLE_BAR_SIZE * 0.5f) + 2,
			//	info.font, true, true, SETTINGS::settings.menu_col, name2);

			RENDER::DrawLine(position.x, position.y + WINDOW_TITLE_BAR_SIZE - 20,
				position.x + size.x, position.y + WINDOW_TITLE_BAR_SIZE - 20,
				CColor(0, 255, 97));
		...
		void CheckboxCommand::Draw() {
			...
			RENDER::DrawFilledRect(position.x + 4, position.y + 4,
			...
			if (menu.GetCommandData(GetID()).checkbox_value) {
				RENDER::DrawGradientRectangle(Vector2D(position.x + 4, position.y + 4),
					...
					SETTINGS::settings.menu_col, CColor(0, 0, 0, 200));
		...
		void SliderCommand::Draw() {
			...
			RENDER::DrawFilledRect(position.x, position.y + 4, position.x
				+ size.x, position.y + size.y - 4, CColor(20, 20, 20, 255));
			...
			if (*variable > slider_min) {
				...
				RENDER::DrawGradientRectangle(Vector2D(position.x, position.y + 4),
					Vector2D(filled_x_position - position.x, 6), SETTINGS::settings.menu_col, CColor(0, 0, 0, 255));

				/*
				position.x, position.y + 4, filled_x_position,
				position.y + size.y - 4, SETTINGS::settings.menu_col
				*/
			}
		...
		void TabCommand::Draw() {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				RENDER::DrawGradientRectangle(
					...
					colors[TAB_BODY], CColor(0, 0, 0, 150));

				/*

				position.x + 5, position.y, position.x
				+ clamped_size.x + 11, position.y + clamped_size.y, colors [ TAB_BODY ]

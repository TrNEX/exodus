...
{
	m_pConVar = mopvar::cvar->FindVar(szCVar);
	m_newvalue = newvalue;
...
SDK::MinspecCvar::~MinspecCvar()
{
	if (ValidCvar())
	{
		mopvar::cvar->UnregisterConCommand(m_pConVar);
		m_pConVar->pszName = m_szOriginalName;
		m_pConVar->SetValue(m_OriginalValue);
		mopvar::cvar->RegisterConCommand(m_pConVar);
	}
}
...
SDK::SpoofedConvar::SpoofedConvar(const char* szCVar) {
	m_pOriginalCVar = mopvar::cvar->FindVar(szCVar);
	Spoof();
}
...
SDK::SpoofedConvar::~SpoofedConvar() {
	if (IsSpoofed()) {
		...
		mopvar::cvar->UnregisterConCommand(m_pDummyCVar);
		free(m_pDummyCVar);
		m_pDummyCVar = NULL;
	}
}
...
void SDK::SpoofedConvar::Spoof() 
{
	if (!IsSpoofed() && m_pOriginalCVar) 
	{
		...
		//Register it
		mopvar::cvar->RegisterConCommand(m_pDummyCVar);
